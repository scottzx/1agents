...
export function InboxDetail({
    ...
    // Esc closes forward picker first, then the drawer.
    useEffect(() => {
    ...
    // Reset forward UI when the selected item changes.
    useEffect(() => {
        setForwardOpen(false);
        setTargets([]);
    }, [item.id]);
    ...
    const openForward = async () => {
        ...
        try {
            ...
            // Cannot forward into the current workspace box.
    ...
    const title = (item.title || item.summary || item.content || item.url || '').trim() || '—';

    return (
        <Fragment>
            <div class="inbox-detail-backdrop" onClick={onClose} />
            <aside
                class="inbox-detail-drawer"
                role="dialog"
                aria-modal="true"
                aria-label={t('inbox.detailAria', language)}
            >
                <header class="inbox-detail-head">
                    <div class="inbox-detail-head-main">
                        <div class="inbox-detail-badges">
                            ...
                            {item.status === 'unread' && (
                                <span class="inbox-detail-status-pill is-unread">
                                    {t('inbox.unreadBadge', language)}
                                </span>
                            )}
                            {archived && (
                ...
                <div class="inbox-detail-body">
                    {item.summary && (
                        <section class="inbox-detail-section">
                            ...
                            <div class="inbox-detail-text">{item.summary}</div>
                        </section>
                    )}
                    ...
                    {item.content && (
                        <section class="inbox-detail-section">
                            ...
                            <div class="inbox-detail-text inbox-detail-content">{item.content}</div>
                        </section>
                    )}
                    ...
                    <section class="inbox-detail-section">
                        ...
                        <div class="inbox-detail-meta-list">
                            <MetaRow label={t('inbox.field.title', language)}>{item.title || null}</MetaRow>
