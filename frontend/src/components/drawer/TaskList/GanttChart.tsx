...
export function GanttChart({ workspaceId, data, initialMilestoneId = '', onMilestoneChange }: GanttChartProps) {
    const [collapsed, setCollapsed] = useState<Set<string>>(new Set());
    const [granularity, setGranularity] = useState<TimeGranularity>('week');
    const [hoveredTask, setHoveredTask] = useState<GanttRow | null>(null);
    const [selectedMilestone, setSelectedMilestone] = useState(initialMilestoneId);

    useEffect(() => {
        setSelectedMilestone(initialMilestoneId);
    }, [initialMilestoneId]);

    const effectiveMilestone = data.milestones.some(milestone => milestone.id === selectedMilestone)
        ? selectedMilestone
        : '';
    const scopedData = useMemo(
        () => filterGanttDataByMilestone(data, effectiveMilestone || undefined),
        [data, effectiveMilestone]
    );
    ...
    const rows = useMemo(() => flattenGanttRows(scopedData.modules, collapsed), [scopedData.modules, collapsed]);
    ...
    return (
        <div class="gantt-chart">
            <div class="gantt-toolbar">
                <label class="gantt-milestone-filter">
                    <span>版本范围</span>
                    <select
                        value={effectiveMilestone}
                        onChange={(event: Event) => {
                            const next = (event.currentTarget as HTMLSelectElement).value;
                            setSelectedMilestone(next);
            ...
            {axis ? (
                <div class="gantt-container">
                    <div class="gantt-labels">
                        <div class="gantt-label-header">模块 / 任务</div>
                        {rows.map(row => (
                            <div
                                key={row.id}
                                ...
                                onClick={() =>
                                    row.type === 'module' ? toggleCollapse(row.id) : handleTaskClick(row.id)
                                }
                                title={row.title}
                            >
                                {row.type === 'module' && (
                    ...
                    <div class="gantt-timeline-wrapper">
                        <svg class="gantt-timeline" width={CHART_WIDTH} height={rows.length * ROW_HEIGHT + ROW_HEIGHT}>
                            ...
                            <g class="gantt-rows-layer" transform={`translate(0, ${ROW_HEIGHT})`}>
                                ...
                                {/* Dependencies Layer */}
                                {rows.map((row, i) => {
                                    if (row.type !== 'task' || !row.dependsOn || row.dependsOn.length === 0)
                                        return null;
                            ...
                            <g class="gantt-milestones-layer" transform={`translate(0, ${ROW_HEIGHT})`}>
                                {scopedData.milestones &&
                                    scopedData.milestones.map(m => {
                ...
                <div class="gantt-chart-empty">
                    {effectiveMilestone
                        ? `${data.milestones.find(milestone => milestone.id === effectiveMilestone)?.version} 暂无已排期数据`
                        : '暂无已排期数据'}
                </div>
            )}

            <GanttUnscheduledTasks tasks={scopedData.unscheduled} onTaskClick={handleTaskClick} />
