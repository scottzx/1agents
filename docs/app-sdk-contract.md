...
> 1agents「AI-native 组织 / 可装可卸多应用平台」的**应用接入规范**。
> 配套总纲见 [`agentsOS-架构设计.md`](./agentsOS-架构设计.md);本文是其 §10「应用契约两面(SDK)」的展开与落地。
> 术语沿用总纲:**任务**是平台基本要素;**核心 / 内核** = ②③④ 框架(出厂就有);**应用 / 专业模板** = ① 业务层 + ④ 里 `function` 的具体实现(长出来、装上去)。
> 适用范围:Epic #317 Phase 1(单用户 · 无 RBAC · 编译期模块 + registry 开关)。

唯一成功标准(北极星):**做第三个应用几乎零内核改动**。本文就是让这件事**可验证**的契约——#347「SDK 契约完备性验证(碰内核 = 0)」拿电台作为第三个应用,严格照此契约落地。
