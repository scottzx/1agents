...
参赛版不再将 Matrix 只定义为 Agent 内部消息总线。Element / Matrix 同时是面向用户的第一方轻量交互入口：

- 用户在聊天中提交目标、追加条件、查看执行摘要和处理人工选择；
- AgentTeams Manager 与 Worker 的协作在 Matrix 中可见；
- 1agents 工作台同步保存 TeamRun、ProjectItem、资产、Evidence、里程碑和发布事实；
- Matrix 消息使用摘要、缩略图和深链接打开 1agents 的任务、文件、Git、资产与预览；
- 聊天记录可作为 Trace 来源，但不代替 1agents 中的权威任务、审批和发布状态。

cc-connect 在该架构中不再是第一方 Agent 聊天主入口，而是保留飞书、Slack、Telegram、钉钉、企业微信等已有平台能力的外部 Channel Gateway。参赛阶段不实现 cc-connect 与 Matrix 的全量双向桥接。

完整的双表面、Action、状态投影和渠道边界见 [Chat-first AgentTeams 交互与渠道架构](../architecture/chat-first-agentteams-interaction.md)。
...
- 参赛阶段的 Matrix 与 cc-connect 全量双向消息同步
