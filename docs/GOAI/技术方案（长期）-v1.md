...
4. **AgentTeams 当前以“吸收设计、预留适配器”为主，不把它的完整分布式基础设施并入默认运行链路。**
5. **MinIO 当前不作为必需服务，先实现本地 AssetStore；多人阶段再换成 MinIO/S3。**
6. **1ACP、cc-connect、Go 后端分别守住执行、交互和任务控制边界，不能互相重复造状态机。**
...
| 1ACP | 启动和恢复 Codex、Claude、Cursor 等执行会话，承载 Agent 运行 | 不成为任务数据库，不拥有项目真相 |
| cc-connect | 外部消息入口、通知、远程审批、结果卡片 | 不成为 Agent 内部协作总线，不保存权威业务状态 |
| Story Studio | 故事、角色、场景、资产、发布、反馈等领域对象 | 不复制通用任务调度 |
| AgentTeams | 未来可选的分布式团队运行时 | 不成为第二套任务/项目控制面 |

cc-connect 当前已经通过较窄的 notifier 边界发送卡片和处理审批，见 [notify.go](/Users/scott/Documents/01-开发项目/1agents/1agents_app/backend/internal/agent/notify.go:10)。这在单人模式下很有价值：人可以离开电脑，由手机接收“请选择悟空角色设定”“是否发布 1.0 内容包”等审批。
...
## 四、AgentTeams 值得吸收什么，不值得现在集成什么

AgentTeams 对长期目标是有价值的，但价值主要在其“团队运行模型”，不在于现在立即部署整套基础设施。
...
但不要现在就为了这个接口改造所有现有逻辑。应先把 LocalTeamRuntime 的真实需求跑通，在多人阶段再实现 AgentTeams provider。
...
这些并不是单人单 PC 阶段的主要矛盾。现在接入，会带来远高于产品收益的维护成本。

### AgentTeams 子模块本身怎么处理

如果未来一至两个主要版本内不会实现 `AgentTeamsRuntime`，不建议让它处于“貌似正式依赖、但实际没有生产使用”的模糊状态。

有两个合理选择。

#### 方案 A：保留，但明确为实验性可选模块

适合你们希望持续跟踪 AgentTeams。
...
从当前 Makefile 情况看，AgentTeams 没有进入默认构建链路，这一点反而是合理的。

#### 方案 B：暂时移除子模块，只保留 ADR 与接口设计

如果短期没有任何运行代码会调用它，这在长期维护上更干净。等多人和远程 worker 需求真实出现时，再以明确版本重新引入。

我的倾向是：

> 如果你们确实会持续研究 AgentTeams，就采用方案 A；否则采用方案 B。不要让一个大型子模块以“以后可能会用”的理由长期成为隐性维护负担。
...
```text
AgentTeamsRuntime implements TeamRuntime
```
...
### 2. AgentRuntime / TeamRuntime

当前实现是 1ACP 本地会话；未来可以增加 AgentTeams 远程实现。
...
当前本地事件和 cc-connect 通知；未来可接 MQ、WebSocket 和外部审计系统。
...
flowchart LR
    ...
    AR -. "当前" .-> LOCAL["Local Team Runtime"]
    AR -. "未来" .-> AT["AgentTeams Runtime"]
```
...
| 1ACP | 是 | 作为 Agent Runtime，不持有任务真相 |
| cc-connect | 是 | 入口、通知、审批，不做内部 Agent 总线 |
| AgentTeams 角色/团队契约 | 是 | 选择性吸收设计 |
| AgentTeams 完整运行栈 | 否 | 多机和多人阶段作为可选 provider |
| MinIO | 暂否 | 先实现 LocalFSAssetStore，未来 S3 backend |
| Matrix | 暂否 | 本地事件和 WebSocket 足够，未来按需适配 |
...
> 以现有 1agents 的 Go 任务内核为控制面，以 1ACP 为本地 Agent 执行面，以 Function Registry 承担确定性媒体操作，以 cc-connect 承担人的远程介入，以 Story Studio 承载绘本领域模型，以 LocalFSAssetStore 和产物血缘建立可追溯生产闭环；等多人、多机需求真实出现后，再把 PostgreSQL、MinIO 和 AgentTeams 作为可替换的服务端与分布式运行后端接入。

这条路线既不会为了未来过早背上分布式基础设施成本，也不会把系统锁死在单机 Demo 阶段。最关键的是，它将长期竞争力放在了真正难以替代的部分：**持久化生产图、版本化成果、结构化交接、验证证据、反馈影响和人机协同决策。**

本轮仅完成了架构与产品分析，没有修改仓库文件。