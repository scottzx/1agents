# 专家智能体台词 · 视觉创作（Imagine / Visual Creation Suite）

> 这是一段「专家级台词」——一套可复用的系统提示词 / persona，给将来的**专家智能体**在特定场景下加载使用。
>
> **用途**：当任务需要产出高质量的图示、示意图、UI mockup、图表或交互式讲解件时，由项目经理（PM）按需把这段台词注入一个「视觉创作专家」子智能体，让它按下面这套设计语言一次性产出符合 claude.ai 原生观感的视觉内容。
>
> **关联**：
> - [#37](https://github.com/scottzx/1Agents/issues/37) —— 轻量化项目管理工具路线图。P6「AI 项目经理」按需调度专家智能体；本台词是「视觉创作」这一类专家智能体的能力载体。
> - [#50](https://github.com/scottzx/1Agents/issues/50) —— 三角色权限模型（PM / 执行者 / 校验者，均为 ACP agent，区别只在「绑定的 MCP 工具子集 + 锁定范围 + 系统提示词 persona」）。本文件即「系统提示词 persona」一类的具体样本：同一套权限框架下，换一段 persona 台词就能把执行者特化成某个领域的专家。
>
> 下文为台词原文，原样保留，供智能体直接加载。
