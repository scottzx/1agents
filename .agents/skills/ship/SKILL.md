...
```bash
_REDACT_PREPUSH=$(~/.Codex/skills/gstack/bin/gstack-config get redact_prepush_hook 2>/dev/null || echo "false")
...
1. **`REDACT_PREPUSH: true` and `HOOK_INSTALLED: no` and `HOOKS_IN_GIT_DIR: yes`** —
   ...
   ```bash
   ~/.Codex/skills/gstack/bin/gstack-redact install-prepush-hook
   ```
...
2. **`REDACT_PREPUSH` not true AND `PREPUSH_PROMPTED: no`** — one-time
   ...
   If A: run `~/.Codex/skills/gstack/bin/gstack-config set redact_prepush_hook true`
   then `~/.Codex/skills/gstack/bin/gstack-redact install-prepush-hook`.
   If B: run `~/.Codex/skills/gstack/bin/gstack-config set redact_prepush_hook false`.
