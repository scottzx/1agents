...
// fakeCLI returns canned message / members JSON depending on the API path.
func fakeCLI(msgsJSON, membersJSON string) CLIRunner {
	return func(ctx context.Context, args ...string) ([]byte, error) {
		for _, a := range args {
			if strings.Contains(a, "/members") {
				return []byte(membersJSON), nil
			}
		}
...
func TestSyncChat_FakeCLI(t *testing.T) {
	...
	msgs := `{"code":0,"data":{"items":[
        {"message_id":"om_1","msg_type":"text","create_time":"1700000001000","sender":{"id":"ou_1"},"body":{"content":"{\"text\":\"hi\"}"}},
        {"message_id":"om_2","msg_type":"post","create_time":"1700000002000","sender":{"id":"ou_2"},"body":{"content":"{\"title\":\"my project\"}"}}
    ]}}`
	members := `{"code":0,"data":{"items":[
        {"member_id":"ou_1","name":"叶子"},
        {"member_id":"ou_2","name":"志伟"}
    ]}}`
	client := newClientWithRunner(fakeCLI(msgs, members), "self")
	syncer := NewSyncer(s, client)

	res, err := syncer.SyncChat(context.Background(), "oc_x")
	if err != nil {
	...
	if got[0].SenderName != "叶子" || got[1].SenderName != "志伟" {
		t.Fatalf("names not enriched: %q %q", got[0].SenderName, got[1].SenderName)
	}
	...
	res2, err := syncer.SyncChat(context.Background(), "oc_x")
	if err != nil {
...
func TestSyncChatLive(t *testing.T) {
	...
	s := openTestStore(t)
	syncer := NewSyncer(s, NewClient("", "self"))
	res, err := syncer.SyncChat(context.Background(), chatID)
	if err != nil {
		t.Fatalf("live SyncChat: %v", err)
	}
	t.Logf("live sync: fetched=%d inserted=%d watermark=%d", res.Fetched, res.Inserted, res.Watermark)
	if res.Inserted == 0 {
	...
	if named == 0 {
		t.Fatalf("no sender names resolved — chat.members enrichment broken")
	}
}
